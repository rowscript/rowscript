...
impl Ecma {
    ...
    fn block(&mut self, sigma: &Sigma, loc: Loc, body: &Box<Term>) -> Result<BlockStmt, Error> {
    ...
    fn expr(&mut self, sigma: &Sigma, loc: Loc, tm: &Box<Term>) -> Result<Box<Expr>, Error> {
        use Body::*;
        use Term::*;
        Ok(match &**tm {
            ...
            Vp(r, ts) => Box::new(Expr::Lit(Lit::Str(JsStr {
                span: loc.into(),
